...
{
    /// <inheritdoc />
    public partial class GetDashboardData2 : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
             var procedure = @"CREATE OR ALTER PROCEDURE [dbo].[GetDashboardData]
                            ...
                            BEGIN
                                SELECT 
                                    (SELECT COUNT(*) FROM Users WHERE (RoleId = 1 OR RoleId = 2) AND OrganizationId = @OrganizationId) AS UserCount,
                                    (SELECT COUNT(*) FROM Users WHERE RoleId = 1 AND OrganizationId = @OrganizationId) AS ManagerCount,
