...
{
    /// <inheritdoc />
    public partial class GetGoodsinFilter : Migration
    {
        /// <inheritdoc />
        ...
        BEGIN
            ...
            BEGIN
                SET @SQL += ' AND G.EntryDate BETWEEN ''' + CONVERT(nvarchar(20), @DateFrom, 120) + ''' AND ''' + CONVERT(nvarchar(20), @DateTo, 120) + '''';
            END
            ELSE IF @DateFrom IS NOT NULL
            BEGIN
                SET @SQL += ' AND G.EntryDate >= ''' + CONVERT(nvarchar(20), @DateFrom, 120) + '''';
            END
            ELSE IF @DateTo IS NOT NULL
            BEGIN
                SET @SQL += ' AND G.EntryDate <= ''' + CONVERT(nvarchar(20), @DateTo, 120) + '''';
            END
